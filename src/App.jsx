...
function useLang() {
  ...
  return { lang, setLang: (v)=>{ setLang(v); setL(v); }, t:(k)=>t(lang,k) };
}
      ...
      <div className="grid cards">
        {notes.map(n => (
          <div className="card" key={n.id}>
            ...
            <div className="muted">{t('lastEdited')}: {new Date(n.updatedAt).toLocaleString()}</div>
            <div className="nav">
              <button className="button" onClick={()=>onEdit(n.id)}>{t('edit')}</button>
              <button className="button danger" onClick={()=>{
                if (confirm(t('confirmDelete'))) { deleteNote(n.id); setNotes(listNotes()); }
              }}>{t('remove')}</button>
            </div>
          </div>
        ))}
...
function Editor({ langApi, id, onHome }) {
  ...
  const timestamp = useMemo(()=> new Date().toLocaleString(),
    [titleTH,titleEN,lyricsTH,lyricsEN,mood,keySig,instrument,notes]);
  ...
  function onSave(){
    ...
    const data = {
      ...
      key: customKey || keySig, mood,
      lyrics_th: lyricsTH, lyrics_en: lyricsEN,
      notes, instrument, updatedAt: now
    };
    saveNote(data); toast(t('updated')); onHome();
  }
  function copyLyrics(){
  ...
  function printPDF(){
    ...
    const html = `<!doctype html><html><head><meta charset="utf-8"><title>${title}</title>
      <style>body{font-family:'Sarabun','Noto Sans Thai',Arial,sans-serif;padding:24px}
      h1{margin:0 0 8px}.muted{color:#666;font-size:12px;margin-bottom:16px}
      pre{white-space:pre-wrap;word-break:break-word}.dev-only{display:none}</style>
...
function toast(msg){
  ...
  Object.assign(el.style,{position:'fixed',left:'50%',bottom:'24px',transform:'translateX(-50%)',
    background:'#000',color:'#fff',padding:'10px 14px',borderRadius:'999px',opacity:'0.9',zIndex:9999});
...
export default function App(){
  ...
  return (
    <div className="app">
      <Header langApi={langApi} onHome={()=>{setView('home'); setEditId(null);}} onNew={()=>{setView('editor'); setEditId(null);}} />
      {view === 'home' && <Home langApi={langApi} onEdit={(id)=>{setView('editor'); setEditId(id);}} />}
